...
function BlogCard({ blog, onLike, onUnlike, onDislike, onUndislike }) {
  return (
    <div key={blog._id} className="bg-white p-6 rounded-lg shadow-lg">
      ...
      <h3 className="text-2xl mb-2">Author: {blog.author}</h3>
      ...
      <div className="flex justify-end items-center mt-4">
        ...
        >
          ...
          {blog.likes > 0 && <span className="ml-1">{blog.likes}</span>}
          {blog.likes === 0 && "Like"}
        </button>
        <button
          onClick={() =>
        ...
        >
          ...
          {blog.dislikes > 0 && <span className="ml-1">{blog.dislikes}</span>}
